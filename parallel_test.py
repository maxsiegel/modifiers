...
def main():
    ...
    block_size = 200
    # results = p.starmap(func_to_iterate_over, list(product(theta_to_test, repeat=5)))
    # parameter_blocks = list(chunked(product(theta_to_test, repeat=5), block_size))[:1000]
    parameter_blocks = list(chunked(product(theta_to_test, repeat=5), block_size))[:10]
    # print(len(parameter_blocks))
    # sys.exit()
    # func_to_iterate_over([(i, shared_array) for i in range(len(theta_to_test))][0])
    # with Pool(10) as p:
        # # results = p.map(func_to_iterate_over, [(i, shared_array) for i in range(len(theta_to_test))])
        # results = p.map(func_to_iterate_over, parameter_blocks)
    # with cfut.SlurmExecutor(additional_setup_lines = ['#SBATCH --cpus-per-task=1']) as executor:
    #executor = cfut.SlurmExecutor()
    executor = cfut.SlurmExecutor()
